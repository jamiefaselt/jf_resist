...
poly <- st_as_sfc(st_bbox(c(xmin = st_bbox(mtwy)[[1]], xmax = st_bbox(mtwy)[[3]], ymax = st_bbox(mtwy)[[4]], ymin = st_bbox(mtwy)[[2]]), crs = st_crs(hsi)))
r <- raster(crs= proj4string(as(poly, "Spatial")), ext=raster::extent(as(poly, "Spatial")), resolution= 540)
hsi.crop <- crop(hsi, as(r, "Spatial"))
plot(mt$geometry)
extent(r)
extent(all.nodes)
st_crs(r)==st_crs(hsi)

writeRaster(r, "temp_rstrmtwy.tif", overwrite = TRUE)
